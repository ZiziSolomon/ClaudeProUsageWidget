...
#     burn 2026-06-08), Haiku own-basis o=6.39 MEASURED 2026-06-11b (paced prose/number
#     crossing, drop-1 6.59; the 06-11a replicate gave 5.14 and the older 06-09 pair
#     7.84 — spread is real, ±~20%) x w_cw1h_haiku=0.31 -> 1.98 Om. Fable own-basis
#     o=5.92 MEASURED 2026-06-10 chain2 (orun_fable_b refit with same-session
#     B=979,815 from brun_fable_b) x w_cw1h_fable=1.49 -> 8.83 Om.
#   cw1h cross-model: cw1h_opus:=1 (gauge), cw1h_sonnet≈1 ASSUMED (the 06-09 "1.016
#     measurement" is VOID — that leg silently ran Opus), cw1h_haiku=0.31 (06-09
#     cross-session bridge — STALE METHOD; the 06-11b same-session bridge leg was
#     lost, re-measure), cw1h_fable=1.49 MEASURED 2026-06-10 SAME-SESSION
#     (B_pp_Om 14,608 ±12% / B_pp_fable 9,798 ±1.5% — carries the Opus leg's 12%).
...
MODEL_WEIGHTS = {
    ...
    "haiku":  {"input": 0.46, "output": 1.98, "cache_write_1h": 0.31, "cache_write_5m": 0.19,  "cache_read": 0.0},
    "fable":  {"input": 2.24, "output": 8.83, "cache_write_1h": 1.49, "cache_write_5m": 0.93,  "cache_read": 0.0},
}
...
IO_UNIT = "weighted_v5"   # basis tag for budgets/anchors; change with any weight
...
def _model_class(model: str) -> str:
    ...
    if "sonnet" in m:
        return "sonnet"
    if "fable" in m or "mythos" in m:   # claude-fable-5 / claude-mythos-5 share weights
        return "fable"
